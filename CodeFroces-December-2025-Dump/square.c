...
void solve() {
    int xCoor[4], yCoor[4];
    for(int i = 0; i < 4; i++) {
        scanf("%d %d", &xCoor[i], &yCoor[i]);
    }

    ll sides;
    if(yCoor[1] == yCoor[0]) {
        sides = abs(xCoor[1] - xCoor[0]);
    } else if(xCoor[1] == xCoor[0]) {
        sides = abs(yCoor[1] - yCoor[0]);
    } else {
        int xCur = xCoor[0], i = 1;
        while(i <= 3) {
            if(xCoor[i] == xCur) break;
            else i++;
        }
        sides = abs(yCoor[i] - yCoor[0]);
    }
    sides *= sides;
    printf("%d\n", sides);
}
